...
reg('alive', { category: 'System', desc: '.alive', run: async ({ reply }) => {
    await reply(`╭━━━〔 *👑 ${BOT_NAME} ACTIVE* 〕━━━┈⊷\n┃ ⚡ *Status:* Online & Stable\n┃ 🤖 *Mode:* Multi-Device Power Bot\n┃ 💎 *Version:* 6.0.0 Ultimate\n┃ ✨ *Owner:* Nimah\n╰━━━━━━━━━━━━━━━━━━━━━━━┈⊷`);
}});
...
reg('owner', { category: 'System', desc: '.owner', run: async ({ reply }) => {
    await reply(OWNER_NUMBER ? `👑 *Owner Contact:* wa.me/${OWNER_NUMBER}` : '👑 Owner number not configured (set OWNER_NUMBER env var).');
}});
...
reg('credits', { category: 'System', desc: '.credits', run: async ({ reply }) => {
    await reply(`✨ *${BOT_NAME}* — built on @whiskeysockets/baileys. Developed by Nimah.`);
}});
...
reg('menu', { category: 'System', desc: '.menu', aliases: ['help'], run: async ({ reply }) => { await reply(buildMenu()); }});
...
reg('would', { category: 'Fun', desc: '.would', run: async ({ reply }) => await reply(`🤔 *Would you rather:*\n\n${pick(WOULD)}`) });
...
// ---- TOOLS ----
reg('calc', { category: 'Tools', desc: '.calc [expression]', run: async ({ reply, q }) => {
    if (!q) return reply('❌ *Provide a math expression!* \n📌 *Example:* `.calc 50 + 50 * 2`');
    try { await reply(`🧮 *Result:* \`${safeCalculate(q)}\` ✨`); }
    catch (e) { await reply('❌ *Invalid mathematical expression!*'); }
}});
reg('time', { category: 'Tools', desc: '.time', run: async ({ reply }) => await reply(`🕐 Current Server Time: ${new Date().toUTCString()}`) });
reg('date', { category: 'Tools', desc: '.date', run: async ({ reply }) => await reply(`📅 Current Date: ${new Date().toDateString()}`) });
reg('base64encode', { category: 'Tools', desc: '.base64encode [text]', aliases: ['b64encode'], run: async ({ reply, q }) => {
    if (!q) return reply('❌ Provide text.'); await reply(`🔐 ${Buffer.from(q).toString('base64')}`);
}});
reg('base64decode', { category: 'Tools', desc: '.base64decode [text]', aliases: ['b64decode'], run: async ({ reply, q }) => {
    if (!q) return reply('❌ Provide base64 text.');
    try { await reply(`🔓 ${Buffer.from(q, 'base64').toString('utf-8')}`); } catch (e) { await reply('❌ Invalid base64 string.'); }
}});
...
reg('reverse', { category: 'Tools', desc: '.reverse [text]', run: async ({ reply, q }) => {
    if (!q) return reply('❌ Provide text.'); await reply(q.split('').reverse().join(''));
}});
reg('upper', { category: 'Tools', desc: '.upper [text]', run: async ({ reply, q }) => { if (!q) return reply('❌ Provide text.'); await reply(q.toUpperCase()); }});
reg('lower', { category: 'Tools', desc: '.lower [text]', run: async ({ reply, q }) => { if (!q) return reply('❌ Provide text.'); await reply(q.toLowerCase()); }});
...
reg('password', { category: 'Tools', desc: '.password [length]', aliases: ['genpass'], run: async ({ reply, q }) => {
    const len = Math.min(Math.max(parseInt(q) || 12, 6), 32);
    await reply(`🔑 Generated Password: \`${genPassword(len)}\``);
}});
...
reg('ai', { category: 'Tools', desc: '.ai [prompt]', aliases: ['gpt'], run: async ({ reply, q }) => {
    ...
    try {
        ...
        await reply(`🤖 *Nimah AI Assistant* ✨\n\n${answer}`);
...
function buildMenu() {
    ...
    let out = `╭━━━〔 *👑 ${BOT_NAME} COMMANDS 👑* 〕━━━┈⊷\n┃ 👋 Hello! Welcome to ${BOT_NAME}\n┃ ⚡ Total Commands Loaded: ${printed.size}+\n┃\n`;
    for (const [cat, list] of Object.entries(grouped)) {
        out += `┃ *[${cat.toUpperCase()}]*\n`;
        list.forEach(d => { out += `┃ 📌 ${d}\n`; });
        out += `┃\n`;
    }
    out += `╰━━━━━━━━━━━━━━━━━━━━━━━━━━━┈⊷`;
    return out;
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            if (statusCode === DisconnectReason.loggedOut) {
                ...
                if (!wasRegistered) {
                    // This almost always means the pairing code expired before
                    // it was entered in WhatsApp (codes are only valid ~60s),
                    // not a real "logged out" event — nothing was ever linked.
                    console.log('⏰ Pairing code expired before it was used. Generate a new one from the pairing page.');
                } else {
